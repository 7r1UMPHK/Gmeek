...
class GMEEK():
    ...
    def markdown2html(self, mdstr):
        ...
        try:
            response = requests.post("https://api.github.com/markdown", json=payload, headers=headers, timeout=60)
    ...
    def fetchEventsParallel(self, issues, workers=16):
        headers = {"Authorization": "token {}".format(self.options.github_token)}
        def one(issue):
            events = []
            url = issue.url + "/events?per_page=100"
            try:
                while url:
            ...
            return issue.number, events
        result = {}
        with ThreadPoolExecutor(max_workers=workers) as ex:
            for num, events in ex.map(one, issues):
                result[num] = events
        return result

    def renderMarkdownParallel(self, items, workers=16):
        def one(pair):
            num, post = pair
            mdFileName = re.sub(r'[<>:/\\|?*\"]|[\0-\31]', '-', post["postTitle"])
            with open(self.backup_dir + mdFileName + ".md", 'r', encoding='UTF-8') as f:
                return num, self.markdown2html(f.read())
        result = {}
        with ThreadPoolExecutor(max_workers=workers) as ex:
            for num, body in ex.map(one, items):
                result[num] = body
        return result
    ...
    def createPostHtml(self,issue,post_body=None):
    ...
    def addOnePostJson(self,issue):
        if len(issue.labels)>=1:
            ...
            events=self.events_map.get(issue.number)
            if events is None:
                events=[e.raw_data for e in issue.get_events()]
            for event in events:
    ...
    def runAll(self):
        ...
        self.events_map=self.fetchEventsParallel(issues)
        for issue in issues:
            self.addOnePostJson(issue)

        rendered=self.renderMarkdownParallel(
            list(self.blogBase["postListJson"].items())+list(self.blogBase["singeListJson"].items()))

        for num,issue in self.blogBase["postListJson"].items():
